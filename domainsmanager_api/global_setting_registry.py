...
ValueKind = Literal["integer", "number", "boolean", "string", "secret", "choice", "json"]
_MISSING = object()
...
def site_setting(
    ...
    *,
    ...
    return GlobalSettingDefinition(
        key, group, label, description, kind, live=True, editor=editor,
...
GLOBAL_SETTINGS = (
    ...
    site_setting("site_name", "站点信息", "站点名称", "用于浏览器标题和页面左上角显示。", default="DomainsManager"),
    site_setting("site_logo", "站点信息", "站点 Logo", "输入图片 URL、站内路径或 SVG 代码。", editor="asset", placeholder="/default.svg 或 <svg>...</svg>", default="/default.svg"),
    site_setting("site_favicon", "站点信息", "站点 Favicon", "输入图片 URL、站内路径或 SVG 代码。", editor="asset", placeholder="/default.svg 或 <svg>...</svg>", default="/default.svg"),
    site_setting("footer_links", "页面配置", "页脚链接", "显示在页面底部的链接列表。", editor="links", default=[]),
    site_setting("footer_copyright", "页面配置", "版权信息", "显示在页面底部的版权文本。", editor="textarea", placeholder="© 2026 DomainsManager"),
    site_setting("icp_number", "页面配置", "ICP 备案号", "显示在页脚并链接至工信部备案查询。"),
    site_setting("police_record_number", "页面配置", "公安备案号", "显示在页脚并链接至公安备案查询。"),
    site_setting("custom_css", "页面配置", "自定义 CSS", "保存后应用于普通页面。", editor="code", language="css", placeholder="/* 自定义样式 */"),
    site_setting("custom_javascript", "页面配置", "自定义 JavaScript", "保存后在普通页面加载完成时执行。", editor="code", language="javascript", placeholder="// 自定义脚本"),
    site_setting("head_html", "页面配置", "头部 HTML", "注入到页面 head 的 HTML 内容。", editor="code", language="html", placeholder="<meta name=\"...\">"),
    site_setting("body_end_html", "页面配置", "底部 HTML", "注入到页面底部的 HTML 内容。", editor="code", language="html", placeholder="<div>...</div>"),
    site_setting("analytics_code", "页面配置", "网站统计代码", "在普通页面加载完成时执行的统计脚本。", editor="code", language="javascript", placeholder="// analytics"),

)

GLOBAL_SETTING_BY_KEY = {definition.key: definition for definition in GLOBAL_SETTINGS}
SITE_SETTINGS = tuple(
    definition for definition in GLOBAL_SETTINGS if definition.group.startswith("站点信息") or definition.group.startswith("页面配置")
)
