...
非敏感、可审计的业务策略可写入数据库；SMTP 密码也可由管理员明文写入并读取；数据库连接、JWT/Token Secret、TLS、CORS、监听与
...
后台设置页面、管理员用户详情/会话操作、全局域名详情操作、运行状态和安全审计查询已完成。OAuth Provider 集成、密码重置和 TOTP 在 M5。后续实现应以
