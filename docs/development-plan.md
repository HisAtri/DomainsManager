...
当前工作基线为 `codex/m4-administration` 的 `758523d`；主分支仍为 `master`。后续状态以代码、契约测试和本文件的验收记录共同为准。
...
- 管理员前端仍以用户列表/封禁解封和全局域名列表为主，缺少用户详情与会话撤销操作，以及全局域名详情、编辑、软删除确认和强制刷新闭环；
- 配置中心已管理账户注册、域名监控、刷新任务、调度、通知和 SMTP 投递共 21 个注册键；设置采用版本控制、审计、SMTP 密码加密入库和运行期重载。
- M4 剩余发布门槛是专用 PostgreSQL 实例上的并发与查询计划验收；仓库默认回归中的 PostgreSQL 用例仍会跳过。
