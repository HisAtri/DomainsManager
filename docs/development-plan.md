...
- 管理员可读取和修改“成功结果新鲜度”策略；该策略已持久化在 `global_setting`，但配置中心尚未完成。
...
- 全局管理员检查历史、失败统计、用户会话单独/全部撤销，以及管理员密码重置工作流尚未完成；
...
与端口、迁移开关、JWT Secret、Refresh Pepper、SMTP 密码及其他通知凭据、TLS、CORS、首次管理员引导凭据。
