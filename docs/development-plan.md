...
当前基线提交为 `59b425e`，主分支为 `master`。后续状态以代码、契约测试和本文件的验收记录共同为准。
...
- 管理员可管理已注册的全局运行设置：账户注册、域名监控、刷新任务、调度、通知和 SMTP 投递。设置写入 `global_setting` 并采用版本控制与审计；SMTP 密码为认证加密记录，API 不回显。
...
- 全局管理员检查历史、失败统计、用户会话单独撤销和后台设置页面已完成；管理员密码重置属于 M5，M4 剩余专用 PostgreSQL 并发与查询计划验收。
- 管理员前端仍以列表和封禁/解封为主，缺少用户详情编辑、域名详情/编辑/删除确认/强制刷新和全局策略设置闭环；
- 全局运行配置只有“成功结果新鲜度”一个持久化键，尚未形成带校验、审计与多进程生效语义的配置中心；
...
| M3 | 已完成 | 定时调度和通知 | M2 |
| M4 | 进行中 | 管理员用户、全局域名管理与全局运行配置中心 | 用户管理依赖 M1；刷新依赖 M2；配置中心依赖管理员授权与审计 |
...
- 补充全局检查历史、失败统计、列表排序/筛选与 PostgreSQL 索引；
...
4. **生效路径。** API、Worker、Scheduler、Notifier 在下一次工作循环或下一次任务领取前读取新值；不要求重启。对于无法热更新的键，API 返回“需重启”，不得伪装为即时生效。
...
- 全局配置仅接受注册表中的非敏感键；非法键、越界值、并发更新均有稳定错误码；
- 配置变更在声明的生效边界内被 API、Worker、Scheduler 和 Notifier 一致采用，并有审计与回滚记录；
- 环境变量和数据库持久化值的优先级、默认值和不支持热更新的键均有文档和测试。
...
| 全局配置的缓存 TTL、主动失效机制与“需重启”策略 | M4 配置中心实现前 | 默认按下一工作循环读取；仅在性能证明需要时引入短 TTL 缓存 |
