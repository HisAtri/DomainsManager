...
## 2026-08-25 — 当前状态对账与 M4 配置中心规划

- 对账确认 M1 已完成；M2/M3 已完成 PostgreSQL 最终验收；M4 已实现管理员授权、用户管理、全局域名与检查记录管理、会话撤销和后台设置中心，阶段状态保持“进行中”。
- M4 的 SQLite/API/前端验收已完成；剩余发布门槛是专用 PostgreSQL 实例上的并发与查询计划验收。
- 新增 [全局运行配置管理](global-configuration-management.md) 方案。`global_setting` 继续只承载非敏感、可审计的业务运行策略；数据库连接、Secret、TLS、CORS 与首次管理员引导继续由部署环境管理。
- 已验证离线回归 `124 passed`（网络和 PostgreSQL 标记用例未在该次运行中执行）以及前端生产构建；全仓 Ruff 格式检查仍有历史格式化债务，CI 的静态检查范围也尚未覆盖全部新增模块。
