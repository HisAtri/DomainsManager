...
export type Domain = { id: string; identity: { ascii_name: string; unicode_name: string; registrable_domain: string; public_suffix: string; tld: string }; monitor_enabled: boolean; renewal_mode: "automatic" | "manual" | "unknown" | null; notes: string | null; registered_at: string | null; expires_at: string | null; registry_updated_at: string | null; dnssec_enabled: boolean | null; version: number; created_at: string; updated_at: string };
export type Page<T> = { items: T[]; page: number; page_size: number; total: number };
export type Check = { id: string; domain_id: string; checked_at: string; duration_ms: number | null; outcome: string; error_code: string | null; error_message: string | null; protocol: "rdap" | "whois" | null; source: string | null; snapshot: Record<string, unknown> | null; changed_fields: string[]; is_stale: boolean; created_at: string };
export type Task = { id: string; status: "queued" | "running" | "success" | "info" | "warning" | "failed"; domain_id: string; domain_name: string; check_id: string | null; error: { code: string; message: string } | null; result: { code: "refreshed" | "data_fresh" | "failed"; message: string | null; source_check_id: string | null; fresh_until: string | null } | null; created_at: string; started_at: string | null; completed_at: string | null; updated_at: string };
export type Settings = { locale: "zh-CN" | "en-US"; timezone: string; default_monitor_enabled: boolean; expiration_warning_days: number[] };
...
export type AdminSession = { id: string; created_at: string; last_seen_at: string; absolute_expires_at: string; revoked_at: string | null; revoke_reason: string | null };
export type GlobalSetting = { key: string; value: number; version: number; source: string; updated_at: string | null };
